/* =========================================================
   map.js — hand-drawn themed SVG map for ONE trip's stops.
   Ported from the old js/maps.js, which flattened every
   adventure in a category into one map; now a trip IS what an
   adventure used to be, so this just draws one ordered stop
   list with a single flag (the trip's own title) on the first
   stop. Kept as a pure string generator (no DOM) exactly as
   before. Exposed as window.TripMap.
   ========================================================= */
(function () {
  const W = 1000, H = 380;
  ...
  const money = (n) => window.Calculator.money(n);
  ...
  function pinSVG(stop, i, vibe, t, flagLabel) {
    const [x, y] = routeXY(t);
    const above = i % 2 === 0;
    ...
    if (flagLabel) {
      ...
      flag = `<text x="${x.toFixed(1)}" y="${flagY.toFixed(1)}" text-anchor="middle" font-family="Caveat, cursive" font-size="19" fill="${vibe.flag}" paint-order="stroke" stroke="${vibe.halo}" stroke-width="4" stroke-linejoin="round">⚑ ${esc(trunc(flagLabel, 22))}</text>`;
    }
  ...
  // stops: this trip's stops, already in display order. tripTitle flags stop 0.
  function buildMapSVG(stops, vibeName, tripTitle, idPrefix, opts) {
    ...
    const pins = stops
      .map((s, i) => pinSVG(s, i, vibe, stops.length === 1 ? 0.5 : 0.06 + 0.88 * (i / (stops.length - 1)), i === 0 ? tripTitle : ""))
      .join("");
  ...
  function update(container, stops, vibeName, tripTitle, idPrefix) {
    if (!container) return;
    const reduce = window.matchMedia && window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    container.innerHTML = buildMapSVG(stops, vibeName, tripTitle, idPrefix, { animate: !reduce });
  }

  window.TripMap = { update, buildMapSVG };
})();
